                    ...
                    actions: [
                      Obx(() {
                        ...
                        return IconButton(
                          onPressed: () {
                            if (isLike) {
                              LikeUtil.instance.unlikeArtist(controller.browseId, adSense: AdSense.playlist_page);
                            } else {
                              LikeUtil.instance
                                  .likeArtist(controller.browseId, controller.info, adSense: AdSense.playlist_page);
                            }
                          },
                          icon: Image.asset(
          ...
          if (tab["tabRenderer"]["selected"] == true) {
            ...
            for (Map oldItem in oldList) {
              try {
                ...
                var cover =
                    oldItem["richItemRenderer"]["content"]["videoRenderer"]["thumbnail"]["thumbnails"].last["url"];
                newList.add({
                  "title": title,
                  "subtitle": "",
                  "cover": cover,
          ...
          if (tab["tabRenderer"]["selected"] == true) {
            ...
            for (Map oldItem in oldList) {
              try {
                ...
                var cover = oldItem["richItemRenderer"]["content"]["playlistRenderer"]["thumbnails"][0]["thumbnails"]
                        .last["url"] ??
                    "";
    ...
    if (result3.code == HttpCode.success) {
      ...
      for (Map tab in tabs) {
        try {
          if (tab["tabRenderer"]["selected"] == true) {
            //获取数据
            List oldList = tab["tabRenderer"]["content"]["sectionListRenderer"]["contents"][0]["itemSectionRenderer"]
            ...
            for (Map oldItem in oldList) {
              try {
                ...
                var cover = oldItem["lockupViewModel"]["contentImage"]["collectionThumbnailViewModel"]
