...
class AdUtils {
  ...
  Future loadAd(AdPosId adPosId, {required AdSense adSense, bool forceLocalJson = false}) async {
    ...
    try {
      ...
      for (var item in configList) {
        ...
        if (source == "admob") {
            ...
            NativeAd nativeAd = NativeAd(
              ...
              listener: admob.NativeAdListener(
                ...
                onAdClicked: (ad) {
                  ...
                  if (adSense == AdSense.cold || adSense == AdSense.first) {
                    EventUtils.instance.addEvent("open_ad_click", data: {
                      "appearance": bus.isFirstAppLaunch ? "first" : "cold",
                      "kid": "ad_click",
                    });
                  }
          ...
          if (type == "open") {
            MaxUtils.instance.bindLoadCallbacks(
              adType: "open",
              adUnitId: ad_id,
              onLoaded: (ad) {
        ...
        loadTimer.cancel();
        loadTimer = null;
        if (source == "max" && (type == "open" || type == "interstitial" || type == "rewarded")) {
          MaxUtils.instance.clearLoadCallbacks(adType: type, adUnitId: ad_id);
        }
        if (isLoadSuc) {
          EventUtils.instance.addEvent(
            "ad_return_sucess",
      ...
      AppLog.i("广告加载结束：$key, $_adLoadingMap");
    }
  }

  Future<bool> showAd(AdPosId adPosId,
      {required AdSense adSense, required AdFunction adFunction, bool forceLocalJson = false}) async {
    if (adIsShowing) {
      AppLog.e("广告正在展示中");
    ...
    for (var item in configList) {
      ...
      if (source == "admob") {
        ...
        if (type == "open") {
          ...
          if (isReady ?? false) {
            MaxUtils.instance.bindShowCallbacks(
              adType: "open",
              adUnitId: ad_id,
              onDisplayed: (ad) {
                adIsShowing = true;
    ...
    //           }
    //           if (adSense == AdSense.open_cool || adSense == AdSense.open_first) {
    //             EventUtils.instance.addEvent(
    //               "open_ad_show",
    ...
    //           setShowTime();
    //           //重新加载一轮广告
    //           loadAd(adPosId, adSense: (adSense == AdSense.open_cool || adSense == AdSense.open_first) ? AdSense.open_hot : adSense);
    //
    //           if (onShow != null) {
    ...
    if (source == "max" && (type == "open" || type == "interstitial" || type == "rewarded")) {
      MaxUtils.instance.clearShowCallbacks(adType: type, adUnitId: ad_id);
    }
    if (!isSuc) {
